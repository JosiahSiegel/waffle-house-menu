...
test("ui: perf — render() chunks sections via requestAnimationFrame (TBT fix)", () => {
  // REGRESSION: previously, render() built one big HTML string
  // (18k+ DOM elements) and assigned it to menuEl.innerHTML in a
  // single shot. The browser parsed it synchronously, creating a
  // 1,000+ms long task (Lighthouse TBT = 1,110ms on real hardware).
  // The fix splits the per-section HTML at <details> boundaries
  // and appends each section in its own task, yielding with
  // requestAnimationFrame between chunks. Each task stays under
  // the 50ms long-task threshold, so TBT drops to near-zero.
  assert.match(
    indexHtml,
    /sectionChunks\s*=\s*html\.split\(/u,
    "render() must split sections (the actual regex literal doesn't matter; the split is the key)",
  );
  assert.match(
    indexHtml,
    /html\.split\([^)]*<details/u,
    "section-split regex must be at <details> boundaries",
  );
  assert.match(
    indexHtml,
    /requestAnimationFrame\(\s*appendNextBatch\s*\)/u,
    "render() must yield to the browser with requestAnimationFrame between chunks",
  );
  assert.match(
    indexHtml,
    /function\s+cacheSection\s*\(\s*sec\s*\)\s*\{[\s\S]*?_itemEls\s*=\s*Array\.from\(\s*sec\.querySelectorAll\(['"]\.item['"]\)\s*\)/u,
    "Per-section cache build must be in a callable function (extracted from the chunked loop)",
  );
  // onRenderComplete must run the post-render work that used to
  // be in the main init block: jumpnav, structured data, clear-all,
  // scroll handler, filter pass, loading hide.
  assert.match(
    indexHtml,
    /function\s+onRenderComplete\s*\(\s*\)\s*\{[\s\S]*?buildJumpnav\s*\(\s*\)[\s\S]*?applyFilters\s*\(\s*\)/u,
    "onRenderComplete must run the post-render work (jumpnav, filter pass)",
  );
  // Critical: the old synchronous innerHTML assignment to a big
  // string must NOT be in the render path. If `menuEl.innerHTML =
  // pinnedHTML + html` reappears, the TBT regression returns.
  assert.doesNotMatch(
    indexHtml,
    /menuEl\.innerHTML\s*=\s*pinnedHTML\s*\+\s*html/u,
    "render() must NOT assign the full html string in one shot (TBT regression)",
  );
});
